...
function buildFontCss(fontDirPath) {
  ...
  for (const d of defs) {
    ...
    css += `@font-face{font-family:'${d.family}';src:url(data:font/ttf;base64,${b64}) format('truetype');font-weight:${d.weight};font-style:${d.style};font-display:block;}
`;
  }

  // Ensure SVG uses the embedded fonts and does NOT fall back per-glyph.
  css += `svg text{font-kerning:normal;text-rendering:geometricPrecision;}
`;
  return css;
}
...
export async function generatePdf(input) {
  ...
  // Helper: shrink font size for long strings so text never clips.
  const fitSize = (text, basePt, minPt, maxChars) => {
    const s = String(text ?? "");
    const n = s.length;
    if (n <= maxChars) return basePt;
    const scale = maxChars / Math.max(n, 1);
    const pt = basePt * scale;
    return Math.max(minPt, Math.round(pt * 100) / 100);
  };
  ...
  const referenceLine = values.referenceNumber
    ? `Our Reference Number: ${values.referenceNumber}`
    : "Our Reference Number:";

  const sizes = {
    mainTitleSize: fitSize(values.mainTitle, 23.04, 16, 28),
    subOneSize: fitSize(values.subOne, 21.96, 16, 26),
    subTwoSize: fitSize(values.subTwo, 21.96, 16, 26),
    headlineSize: fitSize(values.headline, 14.04, 12, 40),
    dateTextSize: fitSize(values.dateText, 9.0, 7.5, 32),
    projectNameSize: fitSize(values.projectName, 14.04, 11, 34),
    loanAmountSize: fitSize(values.loanAmount, 14.04, 11, 20),
    referenceLineSize: fitSize(referenceLine, 8.5, 7, 55),
    companyLineSize: fitSize(values.companyLine, 8.25, 6.5, 70),
  };
  ...
  coverSvg = coverSvg
    ...
    .replaceAll("{{referenceLine}}", escapeXml(referenceLine))
    .replaceAll("{{mainTitleSize}}", String(sizes.mainTitleSize))
    .replaceAll("{{subOneSize}}", String(sizes.subOneSize))
    .replaceAll("{{subTwoSize}}", String(sizes.subTwoSize))
    .replaceAll("{{headlineSize}}", String(sizes.headlineSize))
    .replaceAll("{{dateTextSize}}", String(sizes.dateTextSize))
    .replaceAll("{{projectNameSize}}", String(sizes.projectNameSize))
    .replaceAll("{{loanAmountSize}}", String(sizes.loanAmountSize))
    .replaceAll("{{referenceLineSize}}", String(sizes.referenceLineSize))
    .replaceAll("{{companyLineSize}}", String(sizes.companyLineSize))
  ...
  const page2Html = `<div class="page page-2"><div class="pad"></div></div>`;
  ...
  try {
    ...
    // Ensure webfonts are fully loaded before PDF rendering
